...
def build_section_2(story, s):
    ...
    story.append(Paragraph(
        "Il ruolo dell'esperto di dominio (in questo caso il medico) è cruciale: è lui a "
        "fornire la conoscenza diagnostica che alimenta la KB. Il motore inferenziale di "
        "SWI-Prolog si occupa poi di attivare le regole in modo <i>goal-driven</i> "
        "(backward chaining) per rispondere alle query dell'utente, mentre la GUI Python "
        "svolge il ruolo di interfaccia tra l'utente non-tecnico e il motore logico.", s['BodyJ']))
    story.append(hr())
...
def build_section_5(story, s):
    ...
    story.append(Paragraph(
        "Il motore inferenziale di Prolog risolve i goal in modo <b>goal-driven</b>: a "
        "partire dal goal viene cercato un fatto o una regola la cui testa unifica con "
        "esso, e si tenta ricorsivamente di dimostrare le precondizioni nel body. Questo "
        "schema corrisponde al <i>backward chaining</i> della logica del primo ordine, "
        "paradigma per cui Prolog è stato pensato fin dalla sua origine.", s['BodyJ']))
    story.append(Paragraph(
...
def build_section_6(story, s):
    ...
    story.append(Paragraph("6.2 Flusso di Interazione", s['SectionH2']))
    story.append(Paragraph(
    ...
    story.append(Paragraph("6.3 Architettura del Codice Python", s['SectionH2']))
    story.append(Paragraph(
...
def build_section_8(story, s):
    ...
    story.append(Paragraph(
        "In conclusione, MedExpert AI rappresenta un'applicazione concreta dei principali "
        "temi affrontati nel corso: <b>rappresentazione della conoscenza</b> in una KB come "
        "insieme di sentence, <b>inferenza nella logica del primo ordine</b> con <i>forward</i> "
        "e <i>backward chaining</i>, e <b>Prolog</b> come linguaggio basato su clausole di "
        "Horn con motore inferenziale goal-driven. Il progetto realizza l'architettura "
        "classica di un sistema esperto: KB + esperto di dominio (gli autori in veste di "
        "<i>knowledge engineer</i> medico) + motore inferenziale (SWI-Prolog).", s['BodyJ']))
