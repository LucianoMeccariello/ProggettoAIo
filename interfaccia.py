#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# MedExpert AI - GUI in Tkinter che parla con Prolog.
# Progetto per il corso di Elementi di IA (Universita' Federico II di Napoli).
...
# Palette colori del tema (scuro, blu/petrolio).
BG_ROOT  = '#0a1628'
BG_PANEL = '#0f1f35'
BG_CARD  = '#152840'
BG_INPUT = '#1a3050'
...
# Ponte verso swipl: avvia il processo, lancia la query e legge stdout.
class PrologEngine:
    ...
    @staticmethod
    def _find_swipl():
        # Su Mac/Linux swipl puo' stare in posti diversi a seconda di come
        # e' stato installato. Proviamo i piu' comuni.
    ...
    def _run(self, goal):
        # Lancia swipl in modalita' non interattiva.
        try:
            r = subprocess.run(
    ...
    def diagnosi(self, sintomi):
        # Ritorna una lista di tuple (malattia, certezza) ordinata dalla piu'
        # certa alla meno certa. Se non ci sono sintomi, lista vuota.
        if not sintomi:
            return []
    ...
    def spiega(self, malattia, sintomi):
        # Chiede a Prolog i dettagli per una specifica malattia: sintomi trovati,
        # mancanti, descrizione e trattamento. Tutto in un dizionario.
        sintomi_str = ','.join(sintomi)
...
def round_rect(canvas, x1, y1, x2, y2, radius=12, **kwargs):
    # Disegna un rettangolo con angoli arrotondati su un Canvas Tk.
    # Non c'e' una primitiva, quindi costruiamo un poligono smooth.
    r = radius
    points = [
        x1 + r, y1,   x2 - r, y1,
        x2, y1,       x2, y1 + r,
...
# Classe principale: gestisce la finestra e tutti i widget.
class MedExpertApp:
    # Animazione della barra di certezza nelle card dei risultati.
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Centriamo la finestra a schermo.
        w, h = 1200, 800
        ...
        # Scorciatoie per il fullscreen.
        self._fullscreen = False
        self.root.bind('<F11>', lambda e: self._toggle_fullscreen())
        self.root.bind('<Escape>', lambda e: self._exit_fullscreen())

        # Font: ne definiamo parecchi cosi' non li ricreiamo ogni volta.
        ...
        # Il "motore" che parla con Prolog.
        self.engine = PrologEngine()

        # Stato dell'app.
        self.symptom_vars: dict[str, tk.BooleanVar] = {}   # checkbox dei sintomi
        self.results: list[tuple[str, int]] = []           # ultima diagnosi
        self.selected_disease: str | None = None           # card cliccata
        self._category_open: dict[str, bool] = {}          # se la categoria e' aperta
        self._category_widgets: dict[str, list[tk.Widget]] = {}
        self._card_canvases: list[tk.Canvas] = []
        self._anim_jobs: list[str] = []                    # job di after() in corso

        # Costruzione interfaccia: header, corpo, footer.
        self._build_header()
        self._build_body()
        self._build_footer()

    # Header in alto: titolo, sottotitolo, riga corso.
    def _build_header(self):
        hdr = tk.Frame(self.root, bg=BG_PANEL, padx=20, pady=12)
        hdr.pack(fill='x', side='top')

        # Linea colorata sopra (effetto "accent").
        ...
        # Riga sottile in basso, fa da divisore con il body.
        tk.Frame(hdr, bg=BORDER, height=1).pack(fill='x', side='bottom')

    # Body: 3 colonne (sintomi | risultati | dettagli).
    ...
    # Colonna sinistra: lista checkbox dei sintomi (scrollabile).
    def _build_left_panel(self, parent):
        ...
        # Canvas + scrollbar per fare lo scroll della lista (Tk non lo fa di suo).
        ...
        # Rotellina del mouse (Win/Mac) e tasti Button-4/5 (Linux/trackpad).
    ...
    def _populate_symptoms(self):
        # Tieni traccia di ogni categoria con il suo header, contenuto e arrow.
        # Serve poi per ripopolare nell'ordine giusto quando si chiude/riapre.
        ...
        for cat_label, symptoms in SYMPTOM_CATEGORIES:
            ...
            # Header cliccabile della categoria (con freccia + nome + badge).
            ...
            # Pallino con il numero di sintomi selezionati nella categoria.
            ...
            # Funzione di toggle: apre/chiude il pannellino dei sintomi.
            # Quando riapriamo dobbiamo rimpacchettare tutto per non scombinare l'ordine.
            ...
            # Effetto hover: cambia leggermente lo sfondo al passaggio del mouse.
    ...
    def _repack_all_categories(self):
        # Spacchetta tutto e ripacchetta in ordine, cosi' una categoria che
        # viene riaperta non finisce sotto le altre.
    ...
    def _update_cat_count(self, cat_label, count_var):
        # Aggiorna il numero nel badge di una categoria.
        cat_symptoms = []
    ...
    # Colonna centrale: lista delle diagnosi (card scrollabili).
    def _build_center_panel(self, parent):
        ...
        # Stesso pattern del pannello sinistro: canvas + frame interno scrollabile.
        ...
        # All'avvio mostriamo il messaggio "nessuna analisi effettuata".
    ...
    def _display_results(self, results: list[tuple[str, int]]):
        # Pulisce il pannello centrale e ci mette dentro una card per ogni diagnosi.
        for w in self._res_inner.winfo_children():
            w.destroy()
        self._card_canvases.clear()
        # Eventuali animazioni in corso vanno fermate.
        ...
        # Fa in modo che il frame interno occupi sempre la larghezza del canvas.
    ...
    def _create_diagnosis_card(self, malattia: str, certezza: int, index: int):
        # Una singola card: nome malattia, categoria, barra di certezza animata.
        card = tk.Frame(self._res_inner, bg=BG_CARD, padx=16, pady=14,
                        highlightbackground=BORDER, highlightthickness=1)
        card.pack(fill='x', pady=6)

        # Nome malattia (in alto).
        ...
        # Categoria + colore in base alla certezza (verde/giallo/rosso).
        ...
        # Barra di certezza con percentuale a destra.
        ...
        # Anima la barra da 0% al valore reale.
        self._animate_bar(bar_canvas, pct_label, certezza, cat_color, index)

        # La card e' interamente cliccabile: facciamo bind anche su tutti i figli
        # (altrimenti cliccando sull'etichetta non scatta nulla).
        ...
        # Hover: la card si illumina quando ci passi sopra.
        def on_enter(e):
    ...
    def _animate_bar(self, canvas: tk.Canvas, pct_label: tk.Label,
        ...
        # Anima la barra di certezza da 0% al valore finale, con curva ease-out.
        step_delay = self.ANIM_DURATION_MS // self.ANIM_STEPS
        start_delay = index * 80  # ogni card parte un po' dopo, effetto cascata

        def step(current_step):
            if current_step > self.ANIM_STEPS:
                return
            progress = current_step / self.ANIM_STEPS
            # Ease-out cubica: comincia veloce e rallenta verso la fine.
            ...
            if w < 2:
                w = 300  # fallback se Tk non ha ancora fatto il layout
            bar_w = int((w - 4) * current_val / 100)
            if bar_w > 0:
                # Trucco per il gradiente: rettangolo pieno + uno piu' chiaro
                # nella meta' superiore, da' un finto effetto "lucido".
    ...
    def _lighten(hex_color: str, factor: float) -> str:
        # Schiarisce un colore hex (#aabbcc) di una percentuale 0..1.
    ...
    def _guess_category(malattia: str) -> str:
        # Mappa rapida malattia -> categoria leggibile, usata solo per la UI.
        # Usiamo "in" per gestire varianti tipo "covid19" che contiene "covid".
        cat_map = {
    ...
    def _on_card_click(self, malattia: str):
        # Click su una card: lanciamo Prolog in un thread per non bloccare la GUI,
        # poi torniamo sul main thread con root.after per aggiornare il pannello.
    ...
    # Colonna destra: dettagli sulla malattia che ha cliccato l'utente.
    ...
    def _populate_details(self, malattia: str, info: dict):
        # Riempie il pannello di destra con: descrizione, certezza, sintomi
        # trovati/mancanti e trattamento. Tutto viene da Prolog.
        for w in self._det_inner.winfo_children():
            w.destroy()
        ...
        # Se Prolog non ha dato niente, mettiamo un messaggio neutro.
    ...
    # Footer in basso: bottoni Analizza / Reset / Fullscreen e barra di stato.
    def _build_footer(self):
        ...
        # Barra di stato a destra: messaggi tipo "in corso...", "pronto", ecc.
    ...
    def _on_analyze(self):
        # Avvia la diagnosi in un thread separato per non congelare la finestra
        # mentre swipl gira (puo' impiegare anche qualche centinaio di ms).
        sintomi = self._get_selected_symptoms()
        if not sintomi:
    ...
    def _on_analysis_done(self, results):
        # Callback eseguita nel main thread quando swipl ha finito.
    ...
    def _on_reset(self):
        # Toglie tutte le spunte e ripulisce i pannelli.
...
def main():
    ...
    # Scaling: su monitor HiDPI la finestra altrimenti viene minuscola.
    try:
